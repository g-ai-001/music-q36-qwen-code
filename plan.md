...
- **0.4.1** (已发布) - 修复 CI 构建失败问题（Room DAO 查询返回 Map 类型错误） ✅
- **0.4.2** (已发布) - 修复MusicService生命周期管理问题 ✅
- **0.5.0** (开发中) - 歌单管理功能（创建、编辑、删除）

### 目标
修复 GitHub Actions CI 构建失败问题，确保 workflow 无报错。

### 任务清单
- [ ] 诊断并修复 GitHub Actions workflow 构建失败问题
- [ ] 确保 GitHub Actions workflow 无报错
- [ ] 更新 versionName 到 0.4.2
